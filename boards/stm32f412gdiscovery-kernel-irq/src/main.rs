...
use kernel::hil::gpio::{self, ClientWithValue, Configure, InterruptWithValue, Output};
...
unsafe fn start() -> (
    ...
    let gpio_pins = components::gpio_component_helper!(
        stm32f412g::gpio::Pin,
        // Arduino like RX/TX
        0 => base_peripherals.gpio_ports.get_pin(stm32f412g::gpio::PinId::PG13).unwrap(), //D2
        1 => base_peripherals.gpio_ports.get_pin(stm32f412g::gpio::PinId::PF04).unwrap(), //D3
        2 => base_peripherals.gpio_ports.get_pin(stm32f412g::gpio::PinId::PG12).unwrap(), //D4
        3 => base_peripherals.gpio_ports.get_pin(stm32f412g::gpio::PinId::PF10).unwrap(), //D5
        4 => base_peripherals.gpio_ports.get_pin(stm32f412g::gpio::PinId::PF03).unwrap(), //D6
        5 => base_peripherals.gpio_ports.get_pin(stm32f412g::gpio::PinId::PG11).unwrap(), //D7
        6 => base_peripherals.gpio_ports.get_pin(stm32f412g::gpio::PinId::PG10).unwrap(), //D8
        7 => base_peripherals.gpio_ports.get_pin(stm32f412g::gpio::PinId::PB08).unwrap(), //D9
        // SPI Pins
        8 => base_peripherals.gpio_ports.get_pin(stm32f412g::gpio::PinId::PA15).unwrap(), //D10
        9 => base_peripherals.gpio_ports.get_pin(stm32f412g::gpio::PinId::PA07).unwrap(),  //D11
        10 => base_peripherals.gpio_ports.get_pin(stm32f412g::gpio::PinId::PA06).unwrap(),  //D12
        11 => base_peripherals.gpio_ports.get_pin(stm32f412g::gpio::PinId::PA15).unwrap()  //D13
            ...
            let buf = {
                static mut BUF: (
                    core::mem::MaybeUninit<
                        [Option<&'static InterruptValueWrapper<'static, stm32f412g::gpio::Pin>>;
                            NUM_PINS],
                    >,
                    bool,
                ) = (core::mem::MaybeUninit::uninit(), false);
                kernel::utilities::static_init::static_buf_check_used(&mut BUF.1);
                &mut BUF.0
            };
            buf.write([None; NUM_PINS])
        };
        ...
        pins[0] = Some(
            {
                let buf = {
                    static mut BUF: (
                        core::mem::MaybeUninit<InterruptValueWrapper<stm32f412g::gpio::Pin>>,
                        bool,
                    ) = (core::mem::MaybeUninit::uninit(), false);
                    kernel::utilities::static_init::static_buf_check_used(&mut BUF.1);
                    &mut BUF.0
                };
                buf.write(InterruptValueWrapper::new(
                    base_peripherals
                        .gpio_ports
                        .get_pin(stm32f412g::gpio::PinId::PG09)
                        .unwrap(),
                ))
            }
            .finalize(),
        ...
        pins[1] = Some(
            {
                ...
                buf.write(InterruptValueWrapper::new(
                    base_peripherals
                        .gpio_ports
                        .get_pin(stm32f412g::gpio::PinId::PG14)
                        .unwrap(),
                ))
            }
            .finalize(),
    ...
    extra_pins[1].map(|pin| {
        pin.make_output();
        pin.clear();
    });

    struct ClientFunc {
        respond_pin: &'static kernel::hil::gpio::InterruptValueWrapper<
            'static,
            stm32f412g::gpio::Pin<'static>,
        >,
    }

    impl ClientWithValue for ClientFunc {
        fn fired(&self, _value: u32) {
            self.respond_pin.set();

            // static CNT: core::sync::atomic::AtomicUsize = core::sync::atomic::AtomicUsize::new(0);

            // debug!(
            //     "fired {}",
            //     CNT.fetch_add(1, core::sync::atomic::Ordering::SeqCst)
            // );
            // self.respond_pin.clear();
    ...
    let client_func = {
        {
            let buf = {
                static mut BUF: (core::mem::MaybeUninit<ClientFunc>, bool) =
                    (core::mem::MaybeUninit::uninit(), false);
                kernel::utilities::static_init::static_buf_check_used(&mut BUF.1);
                &mut BUF.0
            };
            buf.write(ClientFunc {
                respond_pin: extra_pins[1].take().unwrap(),
            })
        }
    };

    extra_pins[0].map(|pin| {
        pin.make_input();
        pin.enable_interrupts(gpio::InterruptEdge::RisingEdge)
            .unwrap();
        pin.set_client(client_func);
    });

    // GPIO
    let gpio = GpioComponent::new(board_kernel, capsules_core::gpio::DRIVER_NUM, gpio_pins)
